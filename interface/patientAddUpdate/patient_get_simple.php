...
try {
    ...
    $result = $db->query("SELECT * FROM lists WHERE pid = " . intval($pid) . " AND activity = 1 ORDER BY date DESC LIMIT 200");
    if ($result) {
    ...
    $result = @$db->query("SELECT * FROM patient_chief_complaint WHERE pid = " . intval($pid) . " ORDER BY date_created DESC LIMIT 1");
    if (!$result) {
        $result = null;
    }
    if ($result && $result->num_rows > 0) {
    ...
    // 8.5. Get social history data (OPTIONAL)
    ...
    // 9. Get related persons data (OPTIONAL)
    $result = @$db->query("SELECT * FROM patient_related_persons WHERE pid = " . intval($pid) . " ORDER BY id");
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $response['related_persons_data'][] = cleanUtf8($row);
        }
    }

    // 10. Get consent and authorization data (OPTIONAL)
    ...
    // 11. Get review of systems data (OPTIONAL)
    $ros_check = @$db->query("SHOW TABLES LIKE 'patient_review_of_systems'");
    if ($ros_check && $ros_check->num_rows > 0) {
        $result = @$db->query("SELECT * FROM patient_review_of_systems WHERE pid = " . intval($pid) . " LIMIT 1");
        if ($result && $result->num_rows > 0) {
            $response['review_of_systems_data'] = cleanUtf8($result->fetch_assoc());
        }
    }
