...
if (!empty($_GET['set_pid'])) {
    ...
    // Update the session so OpenEMR knows this patient is selected
    $_SESSION['pid'] = $pid;
    $GLOBALS['pid']  = $pid;
}
...
if (!empty($_GET['set_encounter'])) {
    ...
    $GLOBALS['encounter']  = $encounter;
}
elseif (!empty($_GET['encounter'])) {
    $encounter = (int)$_GET['encounter'];
}
elseif (!empty($_SESSION['encounter'])) {
    $encounter = (int)$_SESSION['encounter'];
}
elseif (!empty($GLOBALS['encounter'])) {
    $encounter = (int)$GLOBALS['encounter'];
}
...
<body>
    <div class="w">
        ...
        <p>No patient selected.<br>
            Please select a patient from the calendar or patient finder.</p>
        <p style="margin-top:16px;">
            <a href="<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/patient_file.php">
...
// ── If no encounter, try to find the most recent one ──────

// ── QUERY: Patient Encounter List ──────────────────────────
$encounterList = [];
...
if (!$encounter) {
    ...
    if ($latestEnc) {
        $encounter = (int)$latestEnc['encounter'];
        $_SESSION['encounter'] = $encounter;
        $GLOBALS['encounter']  = $encounter;
    }
}
...
<body>
    <div id="synapta-dashboard" class="syd-with-ai-panel">
        ...
        <!--  PATIENT HEADER BAR                                    -->
        <!-- ══════════════════════════════════════════════════════ -->
        <header class="syd-topbar" style="display:none;">
        ...
        <nav class="syd-sidebar" style="display:none;">
        ...
        <div id="syd-body">
            ...
            <div class="syd-tab-nav">
                ...
                <div class="syd-tab-right">

                    <div class="syd-actions">
                <button class="syd-btn syd-btn-ghost" onclick="sydOpenChart()" style="color:#888780; padding-right: 0px;">📋 Chart</button>
                <button class="syd-btn syd-btn-ghost" onclick="sydOpenPanel('medications')" style="color:#888780; padding-right: 0px;">💊 Rx</button>
               
            </div>

                    <?php
                        echo "<button class='syd-btn syd-btn-ghost' style='color:#888780;' onclick=\"openSynaptaForm($pid)\"> 📑 Intake Form </button>";
                    ?>
            ...
            <div class="syd-enc-bar">
                ...
                <div class="syd-tab-right">
                        <select id="syd-encounter-select" class="syd-encounter-dropdown"
                        onchange="sydChangeEncounter(this.value)" style="margin-right:10px;font-size: 12px; height: 30px;">

                        <?php foreach ($encounterList as $enc): ?>

                        <option value="<?php echo (int)$enc['encounter']; ?>"
                            <?php echo ((int)$encounter === (int)$enc['encounter']) ? 'selected' : ''; ?>>

                            <?php
                    echo date('M d, Y', strtotime($enc['date']))
                        . ' - '
                        . text($enc['reason'] ?: 'Office Visit');
                    ?>
        ...
        <button class="syd-aip-collapse" onclick="sydToggleAIPanel()"
          ...
          if ($vitalsData && $vitalsData['bps'] >= 140) {
              $bullets[] = [
                  'type'  => 'warn',
                  'icon'  => '⚠️',
                  'text'  => 'BP ' . $bp .' '. $vitalsData['bps']. ' — Elevated. Consider medication review.',
              ];
          } else if ($vitalsData && $vitalsData['bps'] > 120) {
            $bullets[] = [
                  'type'  => 'warn',
                  'icon'  => '⚠️',
                  'text'  => 'BP ' . $bp .' '. $vitalsData['bps']. ' — Mild Elevation. Consider medication review.',
              ];
          } else if ($vitalsData && $vitalsData['bps'] < 90) {
            $bullets[] = [
                  'type'  => 'warn',
                  'icon'  => '⚠️',
                  'text'  => 'BP ' . $bp .' '. $vitalsData['bps']. ' — Low. Consider medication review.',
              ];
          } else if ($vitalsData) {
            $bullets[] = [
                  'type'  => 'ok',
                  'icon'  => '✅',
                  'text'  => 'BP ' . $bp .' '. $vitalsData['bps']. ' — within normal range.',
              ];
          }
          ...
          if ($abnormalCount > 0) {
            ...
            $bullets[] = [
                  'type'  => 'ok',
                  'icon'  => '✅',
                  'text'  => 'No abnormal labs on file. Count: ' . ($labCount)?$labCount:$abnormalCount,
              ];
          }
          ...
          if ($allergyCount > 0) {
            ...
            $bullets[] = [
                  'type'  => 'ok',
                  'icon'  => '✅',
                  'text'  => 'No known allergies on file.',
              ];
          }
          ...
          if (!empty($pendingOrders)) {
            ...
            $bullets[] = [
                  'type'  => 'ok',
                  'icon'  => '✅',
                  'text'  => 'No pending orders on file.',
              ];
          }
...
========================= -->
                ...
                <div class="syd-aip-section ai-note fade">
                    ...
                    <div id="syd-draft-note-content">
                        ...
                        <!-- Recording in progress state — shown while mic is active -->
                        <div id="syd-recording-active" style="display:none; text-align:center; padding:16px 8px;">
                            <div style="font-size:2rem; margin-bottom:6px; animation:syd-pulse 1.2s infinite;">🔴</div>
                            <div style="font-size:0.85rem; color:#F4B0B0; font-weight:600; margin-bottom:4px;">
                                Recording in progress
                            </div>
                            <div id="syd-elapsed-display"
                                 style="font-size:1.4rem; font-weight:700; letter-spacing:2px;
                                        color:#fff; margin-bottom:12px;">
                                00:00
                            </div>
                            <div id="syd-words-display"
                                 style="font-size:0.75rem; color:#9CA3AF; margin-bottom:16px;">
                                Waiting for first upload…
                            </div>
                            <button class="syd-dp2-btn"
                                    onclick="sydToggleScribe()"
                                    style="background:#A32D2D; border-color:#A32D2D;">
                                ⏹ Stop Recording
                            </button>
                        </div>
    ...
    function sydChangeEncounter(value) {
        ...
        if (value === '__new__') {
            ...
            window.location.href =
                "<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/encounter/load_form.php?formname=newpatient";
    ...
    function openSynaptaForm(pid) {

        top.restoreSession();

        const url =
            '<?php echo $GLOBALS['webroot']; ?>/interface/patientAddUpdate/synapta_form.php?pid=' + pid;

        if (top.tabManager) {

            top.tabManager.addTab({
                title: 'Synapta Form',
                url: url
            });
