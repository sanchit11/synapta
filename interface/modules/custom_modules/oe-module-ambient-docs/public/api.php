...
try {
    switch ($action) {
        ...
        case 'process_session':
            ...
            // Supports direct API calls that pass patient_id + encounter_id
            // in the body (e.g. from JS after FastAPI transcription path).
            if (!$session) {
                $patientIdFromBody   = (int)($body['patient_id']   ?? 0);
                $encounterIdFromBody = (int)($body['encounter_id'] ?? 0);

                if (!$patientIdFromBody || !$encounterIdFromBody) {
                    http_response_code(404);
                    echo json_encode([
                        'error' => 'Session not found. Pass patient_id and encounter_id to auto-create.',
                    ]);
                    exit;
                }
                ...
                $sessionId  = sqlInsert(
                    ...
                    [$encounterIdFromBody, $patientIdFromBody, $providerId]
                );
                ...
                if (!$session) {
                    http_response_code(500);
                    echo json_encode(['error' => 'Failed to auto-create session record']);
                    exit;
                }
            }
            ...
            //
            // Priority:
            //   1. transcript field in request body  (JS direct path)
            //   2. In-session accumulated transcript (upload_chunk path)
            //   3. Assembled audio file — transcribe via Whisper
            $accumulated = $_SESSION['ambient_transcripts'][$sessionId] ?? null;

            $incomingTranscript = trim($body['transcript'] ?? '');
            if ($incomingTranscript && empty(trim($accumulated['raw_text'] ?? ''))) {
                $accumulated = [
                    'raw_text'    => $incomingTranscript,
                    'diarization' => $body['diarization'] ?? [],
                    'word_count'  => str_word_count($incomingTranscript),
                ];
                $audit->sttCompleted($sessionId, 0, $accumulated['word_count']);
            }

            if (!$accumulated || empty(trim($accumulated['raw_text'] ?? ''))) {
                // No transcript yet — try transcribing assembled audio file
                $chunkDir      = __DIR__ . '/../storage/chunks/session_' . $sessionId;
                $assembledFile = $chunkDir . '/recording.webm';
                ...
                if (!file_exists($assembledFile) || filesize($assembledFile) < 100) {
                    http_response_code(400);
                    echo json_encode([
                        'error' => 'No transcript or audio recording found for this session. '
                            . 'Pass a transcript in the request body or record audio first.',
                    ]);
                    exit;
                }
                ...
                // Remove assembled audio immediately after transcription — PHI
            ...
            // ── Vitals (current + history) ────────────────────────────
            // form_vitals links to encounters via the forms table
            // (forms.form_id = form_vitals.id, forms.formdir = 'vitals').
            // Falls back to the most-recent row by patient if no encounter link.
            // Also fetches last-5 history for trend intelligence.
            $vitals        = [];
            $vitalsHistory = [];
            try {
                $vitalsRow = sqlQuery(
                ...
                // Historical vitals — last 5 dated readings for trend analysis
                $histRes = sqlStatement(
                    "SELECT date, bps, bpd, pulse, temperature,
                            weight, height, BMI, oxygen_saturation, respiration
                     FROM form_vitals
                     WHERE pid = ? AND activity = 1 AND date IS NOT NULL
                     ORDER BY date DESC LIMIT 5",
                    [$patientId]
                );
                while ($hRow = sqlFetchArray($histRes)) {
                    $hBps = $hRow['bps'] ? (string)(int)$hRow['bps'] : '';
                    $hBpd = $hRow['bpd'] ? (string)(int)$hRow['bpd'] : '';
                    $vitalsHistory[] = [
                        'date'              => $hRow['date']              ?? '',
                        'bp_display'        => ($hBps && $hBpd) ? "{$hBps}/{$hBpd}" : '',
                        'pulse'             => $hRow['pulse']             ? (int)$hRow['pulse']             : null,
                        'temperature'       => $hRow['temperature']       ? (float)$hRow['temperature']     : null,
                        'weight_lbs'        => $hRow['weight']            ? (int)$hRow['weight']            : null,
                        'height_in'         => $hRow['height']            ? (int)$hRow['height']            : null,
                        'bmi'               => $hRow['BMI']               ? (float)$hRow['BMI']             : null,
                        'oxygen_saturation' => $hRow['oxygen_saturation'] ? (int)$hRow['oxygen_saturation'] : null,
                        'respiration'       => $hRow['respiration']       ? (int)$hRow['respiration']       : null,
                    ];
                }
            } catch (\Exception $e) {
            ...
            // Includes normal results (abnormal='no') for ruling-out context.
            $labsRecent = [];
            try {
                $labsRes = sqlStatement(
                    "SELECT pr.result_text, pr.result, pr.units, pr.range,
                            pr.abnormal, pr.date, pr.result_status
                     FROM procedure_result pr
                     JOIN procedure_report prpt
                          ON prpt.procedure_report_id = pr.procedure_report_id
                     JOIN procedure_order po
                          ON po.procedure_order_id = prpt.procedure_order_id
                     WHERE po.patient_id = ?
                       AND pr.result_status = 'final'
                     ORDER BY pr.date DESC LIMIT 20",
                    [$patientId]
                );
                while ($lRow = sqlFetchArray($labsRes)) {
                    $labsRecent[] = [
                        'test'     => $lRow['result_text']   ?? '',
                        'result'   => $lRow['result']        ?? '',
                        'units'    => $lRow['units']         ?? '',
                        'range'    => $lRow['range']         ?? '',
                        'abnormal' => $lRow['abnormal']      ?? 'no',
                        'date'     => $lRow['date']          ?? '',
                        'status'   => $lRow['result_status'] ?? '',
                    ];
                }
            } catch (\Exception $e) {
                error_log('AmbientDocs: labs query failed — ' . $e->getMessage());
            }

            // ── Intake form: chief complaint + review of systems ──────
            $intakeForm = (object)[];
            try {
                $ccRow = sqlQuery(
                    "SELECT complaint_text, severity, duration, onset_date,
                            associated_symptoms, pain_score, pain_location,
                            pain_character, pain_worse, pain_better,
                            symptom_onset_type, symptom_condition, symptom_notes
                ...
                $rosRow = sqlQuery(
                    "SELECT symptoms FROM patient_review_of_systems
                     WHERE pid = ? ORDER BY date_modified DESC LIMIT 1",
                    [$patientId]
                );
                $intakeData = [];
                if ($ccRow) {
                    $intakeData['chief_complaint']     = $ccRow['complaint_text']      ?? '';
                    $intakeData['severity']            = $ccRow['severity']            ?? '';
                    $intakeData['duration']            = $ccRow['duration']            ?? '';
                    $intakeData['onset_date']          = $ccRow['onset_date']          ?? '';
                    $intakeData['associated_symptoms'] = $ccRow['associated_symptoms'] ?? '';
                    $intakeData['pain_score']          = $ccRow['pain_score']          ?? '';
                    $intakeData['pain_location']       = $ccRow['pain_location']       ?? '';
                    $intakeData['pain_character']      = $ccRow['pain_character']      ?? '';
                    $intakeData['pain_worse']          = $ccRow['pain_worse']          ?? '';
                    $intakeData['pain_better']         = $ccRow['pain_better']         ?? '';
                    $intakeData['onset_type']          = $ccRow['symptom_onset_type']  ?? '';
                    $intakeData['symptom_notes']       = $ccRow['symptom_notes']       ?? '';
                }
                if ($rosRow && !empty($rosRow['symptoms'])) {
                    $intakeData['review_of_systems'] = array_values(array_filter(
                        explode('|', $rosRow['symptoms'])
                    ));
                }
                $intakeForm = empty($intakeData) ? (object)[] : $intakeData;
            } catch (\Exception $e) {
                error_log('AmbientDocs: intake form query failed — ' . $e->getMessage());
            }
            ...
            try {
                $shRow = sqlQuery(
                    "SELECT tobacco_status, tobacco_amount, tobacco_years,
                            alcohol_status, alcohol_drinks_per_week,
                            drug_status, substances_used,
                            exercise_frequency, exercise_type, exercise_minutes,
                            diet_type, sleep_hours, occupation, social_history_notes
                     FROM patient_social_history WHERE pid = ? LIMIT 1",
                    [$patientId]
                );
                if ($shRow) {
                    $shData = array_filter([
                        'tobacco'         => $shRow['tobacco_status']          ?? '',
                        'tobacco_amount'  => $shRow['tobacco_amount']          ?? '',
                        'tobacco_years'   => $shRow['tobacco_years']           ?? '',
                        'alcohol'         => $shRow['alcohol_status']          ?? '',
                        'drinks_per_week' => $shRow['alcohol_drinks_per_week'] ?? '',
                        'drugs'           => $shRow['drug_status']             ?? '',
                        'substances'      => $shRow['substances_used']         ?? '',
                        'exercise'        => $shRow['exercise_frequency']      ?? '',
                        'exercise_type'   => $shRow['exercise_type']           ?? '',
                        'exercise_min'    => $shRow['exercise_minutes']        ?? '',
                        'diet'            => $shRow['diet_type']               ?? '',
                        'sleep_hours'     => $shRow['sleep_hours']             ?? '',
                        'occupation'      => $shRow['occupation']              ?? '',
                        'notes'           => $shRow['social_history_notes']    ?? '',
                    ], fn($v) => $v !== '' && $v !== null);
                    $socialHistory = empty($shData) ? (object)[] : $shData;
                }
            } catch (\Exception $e) {
                error_log('AmbientDocs: social history query failed — ' . $e->getMessage());
            }

            // ── Last visit ────────────────────────────────────────────
            // Pull assessment + plan from the most-recent prior encounter note.
            $lastVisit = (object)[];
            try {
                $lvRow = sqlQuery(
                    "SELECT cn.final_note, n.ai_soap_note, r.started_at
                     FROM ambient_clinician_notes cn
                     JOIN ambient_ai_notes n         ON n.id       = cn.ai_note_id
                     JOIN ambient_recording_sessions r ON r.id     = n.session_id
                     WHERE r.patient_id = ? AND r.encounter_id != ?
                     ORDER BY r.started_at DESC LIMIT 1",
                    [$patientId, $encounterId]
                );
                if (!$lvRow) {
                    $lvRow = sqlQuery(
                        "SELECT n.ai_soap_note, r.started_at
                         FROM ambient_ai_notes n
                         JOIN ambient_recording_sessions r ON r.id = n.session_id
                         WHERE r.patient_id = ? AND r.encounter_id != ?
                         ORDER BY r.started_at DESC LIMIT 1",
                        [$patientId, $encounterId]
                ...
                if ($lvRow) {
                    $lvJson = $lvRow['final_note'] ?? $lvRow['ai_soap_note'] ?? null;
                    $lvData = $lvJson ? json_decode($lvJson, true) : null;
                    if ($lvData) {
                        $lastVisit = [
                            'date'       => $lvRow['started_at'] ?? '',
                            'assessment' => $lvData['assessment']['clinical_summary']
                                            ?? (is_string($lvData['assessment'] ?? null)
                                                ? $lvData['assessment'] : ''),
                            'plan'       => is_array($lvData['plan'] ?? null)
                                            ? json_encode($lvData['plan'])
                                            : ($lvData['plan'] ?? ''),
                        ];
                    }
                }
            } catch (\Exception $e) {
                error_log('AmbientDocs: last visit query failed — ' . $e->getMessage());
            }
