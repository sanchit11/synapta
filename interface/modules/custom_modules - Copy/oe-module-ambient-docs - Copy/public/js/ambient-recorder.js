...
(function () {
    ...
    async function requestSoapGeneration() {
        ...
        try {
            ...
            const _cs = (jsonResponse.clinical_summary && typeof jsonResponse.clinical_summary === 'object')
                ? jsonResponse.clinical_summary : {};
    ...
    function _buildPreVisitPanel(cs) {
        ...
        if (pastV && presentV) {
            ...
            const tempTrend   = _compareNum(pastV.temperature?.value,        presentV.temperature?.value);
            const o2Trend     = _compareNum(pastV.oxygen_saturation?.value,  presentV.oxygen_saturation?.value);
            ...
            const nTemp = _numVal(presentV.temperature?.value);
            card.appendChild(_cmpRow(
                'Temperature',
            ...
            const nO2 = _numVal(presentV.oxygen_saturation?.value);
            card.appendChild(_cmpRow(
                'O₂ Saturation',
                pO2 !== null ? pO2 + '%' : (pastV.oxygen_saturation?.flag === 'NOT RECORDED' ? 'Not recorded' : null),
                nO2 !== null ? nO2 + '%' : null,
                o2Trend,
                o2Alert
            ));
            // Weight (may only appear in one set)
            const pWt = _numVal(pastV.weight?.value);
            const nWt = _numVal(presentV.weight);
